...
class Operacion (var operando1:Operando, var tipoOperacion:Tipo, var operando2:Operando) extends Operando {
  ...
  override def calculaOperacion() : Int = {
    tipoOperacion match {
      ...
      case ENTRE => calculaOperacionDecimal(_/_)
      case _ => 0
    }
  }

  private def calculaOperacionDecimal(operacion: (Double, Double)=> Double) : Int = {
    if ( Util.tieneDecimales(operacion(operando1.calculaOperacion(), operando2.calculaOperacion())) ) {
      return 0
    } else {
      operacion(operando1.calculaOperacion(), operando2.calculaOperacion()).intValue()
    }
  }
...
object Operacion {
  ...
  def calculaOperacion(tipoOper:Tipo, numeros: Int*) : Int = {
    tipoOper match {
      ...
      case ENTRE => aplicaOperacionDecimal(_/_, numeros.toList)
      case _ => 0
    }
  }
